...
export default function FilterConfig({ drawId, currentFilters }: FilterConfigProps) {
  ...
  const onSubmit = async (data: FilterForm) => {
    const filters: DrawFilters = {
      ...
      excludeKeywords: data.excludeKeywords
        ? data.excludeKeywords.split(',').map((k) => k.trim()).filter(Boolean)
        : undefined,
  ...
  return (
    <div className="card">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-xl font-semibold text-gray-900">Draw Filters & Rules</h2>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        {/* Engagement Filters */}
        <div>
          <h3 className="text-lg font-medium text-gray-900 mb-4">Engagement Requirements</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              />
              {errors.minFollowers && (
                <p className="mt-1 text-sm text-red-600">{errors.minFollowers.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.minLikes && (
                <p className="mt-1 text-sm text-red-600">{errors.minLikes.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.minComments && (
                <p className="mt-1 text-sm text-red-600">{errors.minComments.message}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              />
              <div>
                <div className="font-medium text-gray-900">Require Following</div>
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              />
              <div>
                <div className="font-medium text-gray-900">Duplicate Detection</div>
                <div className="text-sm text-gray-600">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-800">
            <strong>Tip:</strong> Filters help ensure fair draws by removing ineligible
            participants. All filters are optional, but recommended for better results.
          </p>
        </div>
