...
describe("PricingService", () => {
  ...
  describe("getPlatformPrice", () => {
    it("should return base price for web platform", () => {
      const result = pricingService.getPlatformPrice(10.0, Platform.WEB);
    ...
    it("should apply 30% markup for iOS platform", () => {
      const result = pricingService.getPlatformPrice(10.0, Platform.IOS);

      expect(result.platform).toBe(Platform.IOS);
      expect(result.price).toBe(13.0); // 10 * 1.30
      expect(result.currency).toBe("EUR");
      expect(result.originalPrice).toBe(10.0);
    });

    it("should return base price for Android platform", () => {
      const result = pricingService.getPlatformPrice(10.0, Platform.ANDROID);
  ...
  describe("getCreditPackPricing", () => {
    it("should return correct pricing for one_shot pack on web", () => {
      const result = pricingService.getCreditPackPricing(
        "one_shot",
        Platform.WEB,
      );
  ...
  describe("getSubscriptionPricing", () => {
    it("should return correct pricing for monthly subscription on web", () => {
      const result = pricingService.getSubscriptionPricing(
  ...
  describe("calculateIOSStorePrice", () => {
    it("should calculate correct iOS store price", () => {
      const storePrice = pricingService.calculateIOSStorePrice(10.0);
  ...
  describe("getCreditPackDetails", () => {
    it("should return complete credit pack details", () => {
      const details = pricingService.getCreditPackDetails(
        "pack_10",
        Platform.WEB,
      );
      ...
      expect(details?.pricing.price).toBe(15.0);
    });
    ...
    it("should include iOS markup in pricing details", () => {
      ...
      expect(details?.pricing.price).toBe(19.5); // 15 * 1.30
      expect(details?.pricing.originalPrice).toBe(15.0);
    });
  });

  describe("getSubscriptionDetails", () => {
    it("should return complete subscription details for 48h pass", () => {
      const details = pricingService.getSubscriptionDetails(
        SubscriptionPlan.PASS_48H,
        Platform.WEB,
      );
  ...
  describe("validatePlatformPrice", () => {
    it("should validate correct platform price", () => {
      const isValid = pricingService.validatePlatformPrice(
        13.0,
        Platform.IOS,
        10.0,
      );
