...
import { AuthRequest, CreditTransactionType, PaginatedResponse } from '../../types';
...
export const purchaseCredits = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  logger.info(`Credit purchase initiated: ${req.user.id}, package: ${packageId}`);
...
export const cancelSubscription = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  res.status(200).json({
    status: 'success',
    message: 'Subscription cancelled successfully. Credits remain available until end of billing period.'
  });
});
