 ...
 */
export const connectPlatform = asyncHandler(
 ...
 */
export const disconnectPlatform = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    if (!req.user) {
      throw new AppError("User not authenticated", 401);
    }
    ...
    const accountIndex = userAccounts.findIndex(
      (acc) => acc.platform === platformUpper,
    );
 ...
 */
export const listConnectedAccounts = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    if (!req.user) {
      throw new AppError("User not authenticated", 401);
    }
 ...
 */
export const fetchParticipants = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    if (!req.user) {
      throw new AppError("User not authenticated", 401);
    }
    ...
    logger.info(
      `Participants fetched from ${platform}: ${participants.length} by user ${req.user.id}`,
    );
