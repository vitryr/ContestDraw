import { selectWinners, generateRandomSeed, verifyDrawResult } from '../random.service';
...
describe('RandomService', () => {
  ...
  describe('selectWinners', () => {
    ...
    it('should throw error for empty participants', () => {
      expect(() => selectWinners([], 1, false)).toThrow('No participants provided');
    });

    it('should throw error for zero winners', () => {
      expect(() => selectWinners(mockParticipants, 0, false)).toThrow('Number of winners must be at least 1');
    });
  ...
  describe('selectWinners with seed', () => {
    ...
    it('should produce different results with different seeds', () => {
      ...
      const ids1 = winners1.map(w => w.id).sort().join(',');
      const ids2 = winners2.map(w => w.id).sort().join(',');
      // At least the order should be different most of the time
      expect(winners1.map(w => w.id).join(',')).not.toBe(winners2.map(w => w.id).join(','));
    });
  });
  ...
  describe('verifyDrawResult', () => {
    ...
    it('should verify correct draw results', () => {
      ...
      const isValid = verifyDrawResult(mockParticipants, 2, false, seed, winners);
      expect(isValid).toBe(true);
    });

    it('should fail verification for incorrect winners', () => {
      const fakeWinners = [mockParticipants[0], mockParticipants[1]];
      const isValid = verifyDrawResult(mockParticipants, 2, false, seed, fakeWinners);
    ...
    it('should fail verification for wrong number of winners', () => {
      const winners = selectWinners(mockParticipants, 2, false, seed);
      const isValid = verifyDrawResult(mockParticipants, 3, false, seed, winners);
      expect(isValid).toBe(false);
    });

    it('should fail verification for wrong seed', () => {
      const winners = selectWinners(mockParticipants, 2, false, seed);
      const isValid = verifyDrawResult(mockParticipants, 2, false, 'wrong-seed', winners);
      expect(isValid).toBe(false);
    });
  });
