...
export class CertificateService {
  ...
  private async generatePDF(
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.fontSize(11)
        .font('Helvetica')
        .text(`Remove Duplicates: ${data.filters.removeDuplicates ? 'Yes' : 'No'}`)
        .text(`Max Entries Per User: ${data.filters.maxEntriesPerUser || 'Unlimited'}`)
        .text(`Minimum Mentions: ${data.filters.minMentions}`)
        .text(`Required Hashtag: ${data.filters.requiredHashtag || 'None'}`)
        .text(`Required Keywords: ${data.filters.requiredKeywords?.join(', ') || 'None'}`)
        .text(`Verify Following: ${data.filters.verifyFollowing ? 'Yes' : 'No'}`)
