...
const products: ProductConfig[] = [
  {
    name: '48H Pass',
    description: 'Access to Contest Draw platform for 48 hours - Perfect for running quick contests and giveaways',
    prices: [
      {
        nickname: '48h-pass',
        amount: 999, // $9.99
  ...
  },
  {
    ...
    description: 'Full access to Contest Draw with unlimited draws, advanced features, and priority support',
    prices: [
      {
  ...
  },
  {
    ...
    description: 'Full year access to Contest Draw with 2 months free - Best value for power users',
    prices: [
      {
        nickname: 'annual-pro',
  ...
  },
  {
    ...
    description: 'Custom solution for large organizations with dedicated support, white-label options, and API access',
    prices: [
      {
...
async function setupStripeProducts() {
  ...
  const createdProducts: Record<string, { productId: string; priceIds: string[] }> = {};
  ...
  if (createdProducts['48H Pass']) {
    ...
    console.log(`STRIPE_PRICE_PASS_48H=${createdProducts['48H Pass'].priceIds[0]}`);
  }
  if (createdProducts['Monthly Pro Subscription']) {
    console.log(`STRIPE_PRODUCT_MONTHLY=${createdProducts['Monthly Pro Subscription'].productId}`);
    console.log(`STRIPE_PRICE_MONTHLY=${createdProducts['Monthly Pro Subscription'].priceIds[0]}`);
  }
  if (createdProducts['Annual Pro Subscription']) {
    console.log(`STRIPE_PRODUCT_ANNUAL=${createdProducts['Annual Pro Subscription'].productId}`);
    console.log(`STRIPE_PRICE_ANNUAL=${createdProducts['Annual Pro Subscription'].priceIds[0]}`);
  }
  if (createdProducts['Enterprise Plan']) {
    console.log(`STRIPE_PRODUCT_ENTERPRISE=${createdProducts['Enterprise Plan'].productId}`);
    console.log(`STRIPE_PRICE_ENTERPRISE=${createdProducts['Enterprise Plan'].priceIds[0]}`);
  }
  console.log('');
}
